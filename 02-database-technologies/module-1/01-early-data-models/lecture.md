...
A parent can have many children, but a child has exactly one parent, so a tree expresses a **one-to-many** relationship - written `1:M` - and nothing else. Any data that fits "this thing belongs to that thing, and only that thing" fits a tree cleanly.

![The tree structure as drawn in class: root node, child nodes, and the 1:M rule](boards/01-intro-trees.jpeg)
...
A fuller walkthrough of the structure and its terminology is on GeeksforGeeks: [Introduction to Tree Data Structure](https://www.geeksforgeeks.org/dsa/introduction-to-tree-data-structure/).

### 1.2 Where it came from

IBM, working with Rockwell and Caterpillar, built what became **IMS (Information Management System)** to track the bill of materials *[the full parts list for a manufactured thing, broken down into sub-assemblies and their parts]* for the Apollo programme.

The system tracked millions of parts for the Apollo spacecraft and the Saturn V rocket, and it sent its first `READY` message in August 1968 at Rockwell's Space Division. IBM's own account of this is at [ibm.com/history/information-management-system](https://www.ibm.com/history/information-management-system).
...
The **DOM** *[Document Object Model - the browser's in-memory representation of an HTML page]* is a tree. `<html>` is the root, `<body>` is its child, and every element is a node with one parent and any number of children. When you write `parent.appendChild(child)` you are doing tree surgery. MDN sets this out in [Introduction to the DOM](https://developer.mozilla.org/en-US/docs/Web/API/Document_Object_Model/Introduction).
...
![The taxonomic ranks, broadest at the top narrowing to most unique at the bottom](boards/02-tree-example-taxonomy-life.jpeg)

Each step down is a step in granularity. Domain is the broadest grouping; species is the most specific.

A group at any one of these levels is called a **taxon** *[a named group of organisms at any rank - "Mammalia" is a taxon, and so is "Homo sapiens"]*. The plural is **taxa**. So *taxonomy* is the practice, a *taxon* is one group it produces, and *taxa* is more than one of them. Britannica has a short definition at [britannica.com/science/taxon](https://www.britannica.com/science/taxon).

The ranks come from Carl Linnaeus, the Swedish naturalist who set the system out in the 1750s. Class was the broadest rank he proposed; phylum and domain were added later as the system was pushed further up - see [Taxonomy: Ranks](https://www.britannica.com/science/taxonomy/Ranks).

Linnaeus also gave us the naming convention at the bottom. A species is named with two words, its **genus** and its **species**, which is why we are *Homo sapiens*: genus *Homo*, species *sapiens*.

"Human" is the common name for that combination, not part of the scientific classification. Every organism has both - a two-word scientific name that is unique, and whatever the people living near it happen to call it.

### 2.2 Three taxa, drawn out

In class we picked three organisms and walked each one from Kingdom down to Species. We skipped Domain, because all three are in the same one.

![Three taxa drawn as a tree from Animalia down to species, each level labelled with its rank](boards/03-tree-example-cont.jpeg)
...
Both are valid trees. The difference matters when you have to **search** one.

Finding a node means walking the structure, and the two standard ways of walking a tree are **depth-first** (follow one branch all the way down before trying the next) and **breadth-first** (check every node at one level before going down).
...
**Traversal is rigid.** Because there is only one path to any node - down from the root - every lookup is a walk. In a small tree that costs nothing. In a page with ten thousand DOM nodes it costs enough to be worth engineering around.

**React** *[a widely used JavaScript library for building browser interfaces, which you will meet properly on the front-end side of the programme]* is one place that engineering is visible. Rather than searching and rewriting the real DOM directly, React keeps a **virtual DOM** *[a copy of the tree held in memory, compared against the previous version so that only the changed parts are written to the real page]*.
...
![The network model as drawn in class: nodes, edges, directionality and weights, with the limits of the tree noted alongside](boards/04-network-model.jpeg)
...
Train it on enough data and the weights settle into values that produce useful answers. A large language model is this shape with vastly more of everything - more layers, more nodes, more weights - plus architectural machinery on top that we are not covering. The thing being trained underneath is still a weighted network.

## 5. A network worth drawing: airports

The worked example was a transport network of airports. Oslo in the middle, Bergen and Trondheim off it, then London, Paris, Berlin, Madrid, Rome, Istanbul, and across the Atlantic to New York, Toronto and Miami. Nodes are airports. Edges are the routes that actually exist between them.

![The airport network drawn in class, with flight times on the edges and the Paris to Rome link crossed out](boards/05-network-example-airports.jpeg)
...
### 5.4 Aside: the travelling salesman problem

Outside the scope of this module.

Once you have a weighted network, the next question is the cheapest route through it. The famous version of that question is the **travelling salesman problem**: visit every city once, return to the start, minimise the total distance.

It is easy to state and hard to solve. The number of possible routes grows so fast with the number of cities that checking them all stops being feasible at quite small sizes, so in practice you do not search for the guaranteed best answer, you search for a good one. Britannica has an overview at [britannica.com/science/traveling-salesman-problem](https://www.britannica.com/science/traveling-salesman-problem).

## 6. Activity: find a tree and a network
...
The first group classified potatoes by how they cook.

![The potato tree drawn in class: potatoes split into melne, halvkokefaste and kokefaste, with varieties under each](boards/07-potatoe-tree.jpeg)
...
The second example was a **Markov chain**, brought in by a student who had met them in previous study.

![The weather Markov chain drawn in class: six weather states with directed, weighted edges between them](boards/08-network-markov-chain.jpeg)

The nodes are weather states: sunny, cloudy, rainy, snowy, foggy, stormy. The edges are "what happens tomorrow", and the weight on each edge is the probability of that transition.

Given that today is sunny, there is some probability of sunny again tomorrow (the edge from a node back to itself), some probability of cloudy, some of rainy, and so on. The probabilities leaving any one node add up to 1. GeeksforGeeks has a beginner introduction at [geeksforgeeks.org/machine-learning/markov-chain](https://www.geeksforgeeks.org/machine-learning/markov-chain/).
...
The chain only knows today. It has no way to express "it has been sunny for three days running, so rain is getting more likely", because the next state depends only on the current state and nothing before it. That restriction is called the **Markov property**, and it is what makes these chains simple enough to compute with. Getting history into the model means changing the model, not adding more edges.

### 7.3 Game assets: the tree's limit, in production

The third example came from a student who does game development.

![The game assets tree drawn in class: Assets, Models, Player and NPC, each with its own copy of the jump script](boards/09-tree-game-dev-limits.jpeg)

Game engines organise a project as a tree. `Assets` at the root, `Models` beneath it, `Player` and `NPC` beneath that, and under each of those the scripts that give it behaviour - including a `Jump` script.
...
The tree is not doing anything wrong. It is doing exactly what it promises: everything hangs off exactly one parent, so everything can be found by walking down from the root.

That guarantee is why the structure is easy to navigate, and it is the same guarantee that forbids `NPC` from reaching sideways to borrow the player's jump. There is no import, no export, no reference to elsewhere. Down is the only direction.

It is the same limitation as the DOM in section 3, in a different setting. What both cases want is an edge - a link from this node to that node, carrying the meaning "uses". That is not something a tree can express. It is the first thing a network can.

> A hundred NPCs means a hundred copies of the same jump.

## 8. Sources

1. GeeksforGeeks, *Introduction to Tree Data Structure* - [geeksforgeeks.org](https://www.geeksforgeeks.org/dsa/introduction-to-tree-data-structure/)
2. IBM, *Information Management System* - [ibm.com/history/information-management-system](https://www.ibm.com/history/information-management-system)
3. MDN Web Docs, *Introduction to the DOM* - [developer.mozilla.org](https://developer.mozilla.org/en-US/docs/Web/API/Document_Object_Model/Introduction)
4. Britannica, *Taxon* - [britannica.com/science/taxon](https://www.britannica.com/science/taxon)
5. Britannica, *Taxonomy: Ranks* - [britannica.com/science/taxonomy/Ranks](https://www.britannica.com/science/taxonomy/Ranks)
6. Britannica, *Traveling salesman problem* - [britannica.com/science/traveling-salesman-problem](https://www.britannica.com/science/traveling-salesman-problem)
7. Fagforum Potet, *Potetsorter og egnethet i matlaginga* - [potet.no](https://potet.no/fakta-om-potet/potetsorter-og-egnethet-i-matlaginga)
